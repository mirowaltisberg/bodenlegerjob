...
Independent Swiss job discovery site for vacancies with a clear connection to the flooring trade. The production site is [bodenlegerjob.ch](https://bodenlegerjob.ch).
...
The scheduled workflow runs five isolated scrape workers, with at most two workers active at once. Workers receive no Supabase secret and upload short-lived JSON artifacts. A single publisher then validates the complete snapshot, deduplicates it, enforces absolute and retention thresholds, writes only `trade = boden`, prunes only that trade and verifies the stored count and trade-scoped metadata.

The publisher is intentionally disabled by default. A scheduled run can never publish. The workflow-level publish job runs only for a manual `workflow_dispatch` where `confirm_publish` is exactly `PUBLISH` and the repository variable `BODEN_PUBLISHING_APPROVED` is exactly `true`. Leave that variable unset or use any other confirmation for scrape-only runs. Use `--dry-run` for an offline artifact check. Use `--plan` for aggregate, read-only comparison with the current Bodenbelag inventory. Publishing or pruning production rows requires an intentional authorized run.

The absolute snapshot floor is 25 fresh jobs, calibrated conservatively from the complete 35-day trade matrix measured on 20 August 2026. The independent retention gate still requires at least 50% of a larger recent live inventory, so the dynamic floor can only rise.
