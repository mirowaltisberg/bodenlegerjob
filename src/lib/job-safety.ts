...
export function classifyBodenTitle(titleValue: unknown): JobSafetyResult {
  ...
  if (!match) return { disposition: "REJECT", reason: "no-boden-title-signal" };
  ...
  return {
    disposition: "ACCEPT",
    reason: specific ? "specific-boden-title" : "broad-role-with-boden-context",
...
export function isValidBodenJobId(value: unknown): value is string {
  return typeof value === "string" && idPattern.test(value);
}

export function getBodenProfile(profile: JobSafetyProfile) {
