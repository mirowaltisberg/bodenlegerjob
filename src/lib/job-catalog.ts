...
import { classifyBodenTitle, isValidBodenJobId } from "@/lib/job-safety";
...
function scoreScrapedJob(job: ScrapedJob): number {
  return classifyBodenTitle(job.title).disposition === "ACCEPT" ? 10 : -100;
}
...
function matchesQuery(job: JobListing, query: string): boolean {
  ...
  const hasPublicMatch = (queryToken: string) => {
    if (queryToken === "bodenbelag" || queryToken === "boden") {
      return true;
    }
