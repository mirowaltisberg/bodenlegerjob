...
import { classifyBodenTitle, getBodenProfile } from "@/lib/job-safety";
...
export function buildPublicJobCopy(input: PublicJobCopyInput): PublicJobCopy {
  ...
  const classification = classifyBodenTitle(input.title);
  if (
  ...
  const profile = getBodenProfile(classification.profile);
