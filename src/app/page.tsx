...
const homepageMetadata: Metadata = {
  ...
  description:
    "Finde Stellenangebote für Boden-Parkettleger EFZ, Bodenleger, Parkettleger, Plattenleger und Projektleiter Bodenbelag in der Schweiz.",
...
export default async function HomePage({ searchParams }: HomePageProps) {
  ...
  const heroContent = (
    <div className="trade-hero-grid">
      <div>
        <p className="trade-kicker">Material · Handwerk · ganze Schweiz</p>
        <h1 className="trade-display">Bodenlegerjobs.<em>Fläche für Fläche.</em></h1>
        <p className="mt-5 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-lg">Stellen im Schweizer Bodenbelaggewerk — nach Fachgebiet, Ort und Pensum.</p>
      </div>
      <div className="material-atlas" aria-hidden="true"><span className="material-atlas__label">Musteratlas / Fläche 01</span></div>
    </div>
  );
  ...
  return (
    <>
      ...
      {Object.keys(rawSearchParams).length === 0 ? (
        <HomepageLanding initialData={initialData} config={{ hero: heroContent, sectionKicker: "Stellenleitung", sectionTitle: "Aktuelle Bodenbelagstellen", keywordPlaceholder: "Welchen Job suchst du?" }} />
      ) : (
