...
import { BODEN_CITIES, findBodenCity } from "@/lib/bodenleger-cities";
...
export async function generateStaticParams() {
  return BODEN_CITIES.map((city) => ({ city: city.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { city: citySlug } = await params;
  const city = findBodenCity(citySlug);
  if (!city) return { title: "Bodenleger Jobs" };

  const title = `Bodenleger Jobs ${city.name}`;
  const description = `Stelleninserate mit Bodenbelagbezug für ${city.name} und Umgebung. Arbeitsort und Anforderungen im jeweiligen Inserat prüfen.`;
...
export default async function BodenCityPage({ params }: PageProps) {
  const { city: citySlug } = await params;
  const city = findBodenCity(citySlug);
  if (!city) notFound();
      ...
      <main id="main-content" className="bg-background">
        <section className="bg-primary/5 border-b">
          <div className="container mx-auto px-4 sm:px-6 py-10 sm:py-14 max-w-5xl">
            ...
            <p className="text-slate-600 text-lg leading-relaxed mb-4 max-w-3xl">
              Diese Seite zeigt Suchtreffer mit Bodenbelagbezug für {city.name}.
        ...
        <section className="bg-slate-50 border-y">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">
              Bodenbelag-Berufsbezeichnungen in der Suche
            </h2>
        ...
        <section className="container mx-auto px-4 sm:px-6 pb-10 max-w-4xl">
          ...
          <div className="flex flex-wrap gap-2">
            {BODEN_CITIES.filter((item) => item.slug !== city.slug).map((item) => (
              <Link
                key={item.slug}
                href={`/bodenleger-jobs/${item.slug}`}
