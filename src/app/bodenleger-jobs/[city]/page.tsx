...
import { BODENLEGER_CITIES, findBodenlegerCity } from "@/lib/bodenleger-cities";
...
const ROLE_LINKS: { slug: string; label: string }[] = [
  { slug: "bodenleger-efz", label: "Bodenleger EFZ" },
  { slug: "plattenleger-efz", label: "Plattenleger EFZ" },
  { slug: "parkettleger", label: "Parkettleger" },
  { slug: "vinyl-bodenleger", label: "Vinyl-Bodenleger" },
  { slug: "industrieboden-verleger", label: "Industrieboden-Verleger" },
  { slug: "projektleiter-bodenbelaege", label: "Projektleiter Bodenbeläge" },
];

export async function generateStaticParams() {
  return BODENLEGER_CITIES.map((c) => ({ city: c.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { city: citySlug } = await params;
  const city = findBodenlegerCity(citySlug);
  if (!city) return { title: "Bodenleger Jobs" };

  const title = `Bodenleger Jobs ${city.name} 2026 | Stellen für Bodenleger in ${city.name}`;
  const description = `Offene Bodenleger Stellen in ${city.name} (${city.cantonAbbr}). Lohn ${city.salaryBand}, tägliche Updates aus der ${city.region}.`;

  return {
    title,
    description,
    alternates: { canonical: `/bodenleger-jobs/${city.slug}` },
    openGraph: {
      title,
      description,
      url: `/bodenleger-jobs/${city.slug}`,
      type: "website",
      locale: "de_CH",
    },
  };
...
export default async function BodenlegerCityPage({ params }: PageProps) {
  const { city: citySlug } = await params;
  const city = findBodenlegerCity(citySlug);
  if (!city) notFound();
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Startseite", item: SITE_URL },
      {
        "@type": "ListItem",
        position: 2,
        name: `Bodenleger Jobs ${city.name}`,
        item: `${SITE_URL}/bodenleger-jobs/${city.slug}`,
      },
    ],
  };
  ...
  const itemListSchema = {
    ...
    "@type": "ItemList",
    name: `Bodenleger Jobs ${city.name}`,
    description: `Offene Bodenleger Stellen in ${city.name}`,
  ...
  const FAQS = [
    {
      question: `Wie viele Bodenleger Stellen gibt es aktuell in ${city.name}?`,
      answer: `In ${city.name} und Umgebung sind regelmässig zwischen 20 und 120 offene Bodenleger Stellen ausgeschrieben. Die Zahl schwankt mit Saison, Bautätigkeit und Auftragslage. Pendelbereite Bewerbende erweitern den Suchradius typischerweise auf ${city.commuterTowns.slice(0, 3).join(", ")} und finden so 2- bis 3-mal mehr passende Stellen.`,
    },
    {
      question: `Was verdient ein Bodenleger in ${city.name}?`,
      answer: `Ein Bodenleger verdient in ${city.name} typischerweise ${city.salaryBand} pro Jahr (12 × Monatslohn). Berufseinsteiger starten am unteren Ende, mit fünf Jahren Erfahrung und Spezialisierungen verschiebt sich der Lohn deutlich nach oben. Detaillierte Vergleichstabellen findest du auf der Lohnübersicht.`,
    },
    {
      question: `Welche Arbeitgeber suchen Bodenleger in ${city.name}?`,
    ...
    {
      question: `Wie pendle ich am besten zu einem Bodenleger Job in ${city.name}?`,
      ...
      <main className="bg-white">
        <section className="bg-primary/5 border-b">
          <div className="container mx-auto px-4 sm:px-6 py-10 sm:py-14 max-w-5xl">
            <nav className="text-sm text-slate-500 mb-3" aria-label="Breadcrumb">
              ...
              <span className="text-slate-700">Bodenleger Jobs {city.name}</span>
            </nav>
            <h1 className="text-3xl sm:text-5xl font-black text-slate-900 leading-tight mb-4">
              Bodenleger Jobs <span className="text-primary">{city.name}</span>
            </h1>
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
          <h2 className="text-2xl font-bold text-slate-900 mb-4">
            Offene Bodenleger Stellen in {city.name}
          </h2>
                ...
                <Card key={`${job.source}-${job.id}`} className="hover:border-primary/40 transition">
                  <CardContent className="p-4">
                    ...
                    <p className="text-sm text-slate-600 mb-2 line-clamp-1">
                      {job.company || "Schweizer Bodenleger-/Plattenleger-Betrieb"}
                    </p>
        ...
        <section className="bg-slate-50 border-y">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">
              Bodenleger Berufe in {city.name}
            </h2>
        ...
        <section className="bg-slate-50 border-t">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-4xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-5">
              Häufig gestellte Fragen — Bodenleger Jobs {city.name}
            </h2>
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 max-w-4xl">
          ...
          <div className="flex flex-wrap gap-2">
            { BODENLEGER_CITIES.filter((c) => c.slug !== city.slug).map((c) => (
              <Link
                key={c.slug}
