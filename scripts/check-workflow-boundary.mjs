...
const approvalExpression =
  "\${{ github.event_name == 'workflow_dispatch' && inputs.confirm_publish == 'PUBLISH' && vars." +
  expectedVariable +
  " == 'true' }}";
assert(
...
assert(
  publish.if.includes("github.event_name == 'workflow_dispatch'") &&
    !publish.if.includes("schedule"),
  "scheduled events must be unable to satisfy the publisher guard",
);
